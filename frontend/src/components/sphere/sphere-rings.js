// // ─── SPHERE: RINGS (Estilo Orbe Paramétrico de Líneas) ───────────────────────
// // Convertido a fábrica: createRingsSphere(container) -> { setState, destroy }
// // ───────────────────────────────────────────────────────────────────────────

// function createRingsSphere(container) {
//     const SPHERE_SIZES = {
//         IDLE: 0.7,
//         WAKE_DETECTED: 1.1,
//     }
...
//             positions.push(positions[prevIdx], positions[prevIdx+1], positions[prevIdx+2])
...
//             if (state === 'WAKE_DETECTED') targetMorph = 1.0
//             else if (state === 'IDLE') targetMorph = 0.0
//         }
//         },
//         destroy() {
...
// ─── SPHERE: RINGS (Estilo Orbe Paramétrico de Líneas - Siri Edition) ────────
// Convertido a fábrica: createRingsSphere(container) -> { setState, destroy }
// ───────────────────────────────────────────────────────────────────────────
...
function createRingsSphere(container) {
    ...
    // Aumentamos la densidad para un brillo (bloom) más orgánico
    const count = 120
    ...
    const customMaterial = new THREE.ShaderMaterial({
        ...
        void main() {
            ...
            // Movimiento fluido tipo Siri (suave y amplio)
            float noiseFreq = 0.9;
            float noiseTime = uTime * 0.4;
            float n = snoise(pos * noiseFreq + vec3(0.0, 0.0, noiseTime));
            vec3 fluidPos = pos + normalize(pos) * (n * 0.35);

            // Efecto Wake (onda expansiva plana)
            float r = length(pos.xz);
            float wave = sin(r * 12.0 - uTime * 5.0) * 0.1 * (1.0 - r);
            vec3 wakePos = pos + vec3(0.0, 0.0, wave);
        ...
        void main() {
            // Paleta de colores inspirada en Siri
            vec3 cyan = vec3(0.1, 0.8, 1.0);
            vec3 blue = vec3(0.2, 0.4, 1.0);
            vec3 purple = vec3(0.6, 0.1, 0.9);
            vec3 magenta = vec3(1.0, 0.1, 0.6);
            vec3 coreWhite = vec3(1.0, 0.9, 1.0);

            // Mezcla dinámica basada en la posición y el tiempo
            float mix1 = sin(vPos.y * 2.0 + uTime * 0.8) * 0.5 + 0.5;
            float mix2 = cos(vPos.x * 2.0 + uTime * 0.6) * 0.5 + 0.5;
            float mix3 = sin(vPos.z * 1.5 - uTime * 0.5) * 0.5 + 0.5;

            vec3 colorA = mix(cyan, blue, mix1);
            vec3 colorB = mix(purple, magenta, mix2);
            vec3 finalColor = mix(colorA, colorB, mix3);

            // Añadir un núcleo brillante (más fuerte en el centro)
            float coreIntensity = smoothstep(0.9, 0.0, vDistance) * 0.6;
            finalColor = mix(finalColor, coreWhite, coreIntensity);

            // Suavizado en los bordes
            float edgeAlpha = smoothstep(1.5, 0.4, vDistance);

            // Multiplicamos el color para potenciar el AditiveBlending
            gl_FragColor = vec4(finalColor * 1.6, edgeAlpha * 0.85);
        }
        `
    })
